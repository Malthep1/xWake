...
string xWakeSettingsCtrl::createSetting(string setting)
{
	...
	toSet = common::spliceString(setting);
...
bool xWakeSettingsCtrl::getSetting(string name, string& returnString)
{
	map<string, xWakeSetting>::iterator it;
	it = settingsMap.find(name);
	if (it == settingsMap.end())
	...
	returnString = it->second.toString();
	return true;
}
...
{
	vector<string> settings = common::spliceString(setting);
